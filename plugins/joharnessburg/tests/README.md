...
- `tests/scripts/test_emit_event.py` — atomic, append-only event envelopes
- `tests/scripts/test_apply_template.py` — apply_template.py (template merge + safety guards)
- `tests/scripts/test_activate_codex_template.py` — project-local Codex plugin activation and marketplace merge
- `tests/scripts/test_reset_john.py` — reset_john.py (applied-dir reset + metadata guard)
- `tests/scripts/test_john_run.py` — durable run manifests, attempt receipts, reconciliation, retries, and cancellation
