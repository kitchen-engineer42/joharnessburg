...
- **Imperative DSLs beat declarative prose.** When working with weaker workerLLM models, be more imperative and less declarative. mathlab's `ops` DSL is small, each op self-contained, each step builds on the last. The model is much better at this than at producing a free-form game-state JSON.
