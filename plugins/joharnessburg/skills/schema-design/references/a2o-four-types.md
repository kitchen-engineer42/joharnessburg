...
The Anything2Ontology research project (`/Users/mac/Desktop/Anything2Ontology` on the dev machine) modeled knowledge as four distinct types. This was a stronger design than production's single-SKU collapse (see `to-skills-backend-sku-regression.md`).
...
`Anything2Ontology/src/chunks2skus/schemas/sku.py` defines `SKUType` (an enum: FACTUAL, RELATIONAL, PROCEDURAL, META) plus the `LabelTree`, `RelationType`, etc. on the dev machine. The pipeline that uses these types is in `Anything2Ontology/src/chunks2skus/extractors/`.
