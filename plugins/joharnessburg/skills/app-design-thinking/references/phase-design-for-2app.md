...
Per spec §8.5 user reply: *"Closer to phases-as-skills, but leaving room for template to overwrite. We provide the most general level of phase as a suggestive, not enforcing, guideline."*
...
Shapes synthesized from the 5 subsites' pipelines (see `app-archetypes.md`) plus the production `skills2app` 8-stage Celery pipeline (init → coding → review → build → run → health → deploy → success) at `/Users/mac/Desktop/john/production/skills2app/apps/projects/tasks.py` on the dev machine.
