...
5. **Trained skills carry their provenance header.** Any worker skill that went through the training loop ([[skill-evolution]]) ships with its provenance comment (when trained, scorer, baseline → final scores) intact — that header is how the next builder knows the text was earned, not guessed.
6. **No public-label leakage.** Produced skill names and schema keys are internal retrieval handles. The app phase must map them through `.john/contracts/app_blueprint.json` before showing anything to end-users.
7. **The skills-analytics dashboard (if installed) shows the new skills as discoverable.** Optional check, valuable when shaking down a fresh run.
