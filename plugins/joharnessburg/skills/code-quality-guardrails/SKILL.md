...
The produced app is the deliverable. The user trusts it not to leak credentials, not to ship debug noise, not to crash on the first run. This skill is the discipline that makes that trust possible — adapted from skills2app's production quality patterns (spec §8.11 user reply: *"inherit skills2app methods, but again, skill-ify them and teach Claude how to do them via John"*).
...
These are real bugs that grep won't catch. The **cross-validation pattern** (per skills2app's design, spec §8.14): a separate reviewer subagent reads the produced code + design intent + a few key files, returns a flagged-issues list. Opt-in via flag (per skills2app's `AGENT_REVIEW_ENABLED`); slows the pipeline but catches what guardrails miss.
