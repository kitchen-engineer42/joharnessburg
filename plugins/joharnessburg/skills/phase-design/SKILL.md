...
- **Done criteria can name the skill reference they rest on.** Optional, but when a check leans on a specific skill's reference implementation or output shape, naming it (e.g. "done when [[knowledge-extraction]] has emitted events for all chunks") makes the gate auditable from PLAN.md — and if you *can't* name the reference, that's a useful prompt: is the skill genuinely reference-free here, or did the criterion quietly skip it?
