...
There's no single canonical implementation in our prior projects — onion-wrapper was identified during initial John spec discussions (spec §8.5 user reply: *"If input is a large number of short files with several layers of folder, the job should be 'reversed' into 'onion_wrapper'."*). The pattern is documented; M3 ships it as a skill, not a script. Templates that handle wrapper-shaped corpora may ship their own helper scripts.
