...
Spec §4 defines four interlocking structures. They form a cascade: each constrains the next. Getting the order right means the project's shape is internally consistent.
...
Spec §4 introduces the four structures; spec §3 + §8 user replies refine what each means. Plan-md-authoring covers the surface; this note dives deeper into the cascade itself.
